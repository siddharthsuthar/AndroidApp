...
class update_user_info {
    ...
    public function StoreUserSkills($user,$programmingLanguages,$tools,$framework,$databases){
        $stmt = $this->conn->prepare("INSERT INTO skills(user,programmingLanguages, tools, framework, databases) VALUES(?, ?, ?, ?, ?)");

        $stmt->bind_param("ssssss", $user, $programmingLanguages, $tools, $framework, $databases);
        $result = $stmt->execute();
        $stmt->close();
